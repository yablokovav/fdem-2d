...
######################### Calculating displacement for a homogeneous model
barrier_type = 'homogeneous'
...
ut.extract_and_save_displacement(data, nz, npml, rec_indexes, nfreq, barrier_type, 0, 0, dir4result_save)
print()

######################### Calculating displacement for a model with XPS barrier for each configuration
barrier_type = 'xps_horizontal'
for bar_l in barrier_length_all:
    for bar_w in barrier_width_all:
        # preparation of a reference model
        vp, vs, rho, qp, qs = ut.prepare_referent_model(nz, nx, vs_ref, puasson_ref, qp_ref, qp_ref/2, nfreq)
        # addition barrier to reference model
        vp, vs, rho, qp, qs = ut.add_barrier2reference_model(h, vp, vs, rho, qp, qs, nfreq, vp_xps, vs_xps, rho_xps, qp_xps, qp_xps/2, bar_l, bar_w, soux, bar_w/2)
        # calculation elastic equation
        data  = solve_multi_frequency_elastic(frequencies, vp, vs, qp, qs, rho, nx, nz, h, npml, souy, soux, ampl_sou)
        # saving displacement in recievers point
...
barrier_type = 'eps_horizontal'
for bar_l in barrier_length_all:
    for bar_w in barrier_width_all:
        # preparation of a reference model
        vp, vs, rho, qp, qs = ut.prepare_referent_model(nz, nx, vs_ref, puasson_ref, qp_ref, qp_ref/2, nfreq)
        # addition barrier to reference model
        vp, vs, rho, qp, qs = ut.add_barrier2reference_model(h, vp, vs, rho, qp, qs, nfreq, vp_eps, vs_eps, rho_eps, qp_eps, qp_eps/2, bar_l, bar_w, soux, bar_w/2)
        # calculation elastic equation
        data  = solve_multi_frequency_elastic(frequencies, vp, vs, qp, qs, rho, nx, nz, h, npml, souy, soux, ampl_sou)
        # saving displacement in recievers point
        ut.extract_and_save_displacement(data, nz, npml, rec_indexes, nfreq, barrier_type, bar_l, bar_w, dir4result_save)
...
barrier_type = 'rockwool_horizontal'
for bar_l in barrier_length_all:
    for bar_w in barrier_width_all:
        # preparation of a reference model
        vp, vs, rho, qp, qs = ut.prepare_referent_model(nz, nx, vs_ref, puasson_ref, qp_ref, qp_ref/2, nfreq)
        # addition barrier to reference model
        vp, vs, rho, qp, qs = ut.add_barrier2reference_model(h, vp, vs, rho, qp, qs, nfreq, vp_rockwool, vs_rockwool, rho_rockwool, qp_rockwool, qp_rockwool/2, bar_l, bar_w, soux, bar_w/2)
        # calculation elastic equation
        data  = solve_multi_frequency_elastic(frequencies, vp, vs, qp, qs, rho, nx, nz, h, npml, souy, soux, ampl_sou)
        # saving displacement in recievers point
        ut.extract_and_save_displacement(data, nz, npml, rec_indexes, nfreq, barrier_type, bar_l, bar_w, dir4result_save)
